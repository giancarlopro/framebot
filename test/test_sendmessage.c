...
int _message(){
	...
	fflush(stdout);
	result = send_message_chat(_bot, chat_id, "parameter chat_id", 0, 0, NULL);
	if(result){
		printf(BLUE "OK\n" COLOR_RESET);
	}
	else{
	...
	fflush(stdout);
	result = send_message_chat(_bot, chat_id, "parameter parse_mode = MODE_HTML%0Abold = <b>bold</b>, <strong>bold</strong>%0Aitalic = <i>italic</i>, <em>italic</em>%0Agithub = <a href=\"https://github.com/giancarlopro/framebot\">inline URL</a>", 0, 0, NULL);
	if(result){
		printf(BLUE "OK\n" COLOR_RESET);
	}
	else{
	...
	fflush(stdout);
	result = send_message_chat(_bot, chat_id, "parameter parse_mode = MODE_MARKDOWN%0Abold = *bold text*%0Aitalic = _italic text_%0A[inline URL](https://github.com/giancarlopro/framebot)%0A```block_languagepre-formatted fixed-width code block```", 0, 0, NULL);
	if(result){
		printf(BLUE "OK\n" COLOR_RESET);
	}
	else{
	...
	fflush(stdout);
	result = send_message_chat(_bot, chat_id, "parameter disable_web_page_preview https://github.com/giancarlopro/framebot", 1, 0, NULL);
	if(result){
		printf(BLUE "OK\n" COLOR_RESET);
	}
	else{
	...
	fflush(stdout);
	set_notification(1);
	result = send_message_chat(_bot, chat_id, "parameter disable_notification", 1, 0, NULL);
	if(result){
		printf(BLUE "OK\n" COLOR_RESET);
	}
	else{
	...
	fflush(stdout);
	Message * forward = send_message_chat(_bot, chat_id, "parameter reply_to_message_id", 1, result->message_id, NULL);
	if(result){
		printf(BLUE "OK\n" COLOR_RESET);
